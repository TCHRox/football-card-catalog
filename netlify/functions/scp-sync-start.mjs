import {bypassLegacyCooldown,authorized,store,KEY,blankState,json} from './_scp-core.mjs';
export default async request=>{
  ...
  try {
    ...
    if(state.status.retryAfter>Date.now()&&!bypassLegacyCooldown(state.status))return json({error:state.status.error+' Retry after '+new Date(state.status.retryAfter).toISOString()+'.'},429);
